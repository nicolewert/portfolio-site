...
export default function ProjectCard(props: ProjectCardProps) {
  ...
  return (
    <div
      className={`glass w-full rounded-2xl hover:scale-[1.02] hover:shadow-2xl transition-all duration-300 ease-out ${large ? 'min-h-[400px] sm:min-h-[900px]' : 'min-h-[300px] sm:min-h-[540px]'} flex flex-col overflow-hidden`}
    >
      {/* Image carousel */}
      <div
        ...
        title={
          imageCount > 1 ? 'Click or scroll to view next image' : undefined
        }
      ...
      <div
        className={`p-4 sm:p-8 flex flex-col flex-grow ${large ? 'text-lg' : ''}`}
      >
          ...
          {Array.isArray(tags) && tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {tags.map((tag: string) => (
                <span
                  key={tag}
                  className="bg-[var(--primary)]/10 text-[var(--primary)] px-2 py-1 sm:px-3 sm:py-1 rounded-full text-xs font-medium"
                >
                  {tag}
                </span>
              ))}
